...
from app.models import Admin, Tag, Movie, Preview, User, Comment, Moviecol, Userlog, Adminlog, Oplog, Auth, Role
from app import db, app

from . import admin


# 上下文应用处理器(封装全局变量，把它展现在网站中)
...
#权限访问的装饰器
#从session中查找管理员，在查找权限ID，在查找访问路由
def admin_auth(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        ...
        flag = False
        if any(str(rule) in url for url in urls):
            flag=True
...
def movie_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if movie_count == 1 and movie.title != data['movie']:
            flash("片名已经存在", "err")
...
def preview_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if preview.title != data['title'] and preview_count == 1:
            flash("预告已经存在!", "err")  # 闪存错误信息
            return redirect(url_for("admin.preview_edit", id=id))  # 跳转到后台登录页
...
def role_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if role.name != data['name'] and role_count == 1:
            flash("角色已经存在!", "err")  # 闪存错误信息
            return redirect(url_for("admin.role_edit", id=id))  # 跳转到后台登录页
        role.name = data['name']
...
def auth_edit(id):
    ...
    if form.validate_on_submit():
        ...
        if auth.name != data['name'] and auth_count == 1:
            flash("权限已经存在!", "err")  # 闪存错误信息
            return redirect(url_for("admin.auth_edit", id=id))  # 跳转到后台登录页
        auth.name = data['name']
        db.session.add(auth)
        db.session.commit()
        flash("修改标签成功", 'ok')
